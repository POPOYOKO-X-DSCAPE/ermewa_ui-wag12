...
function replaceParamsInString<
	T extends Record<string, string | number>,
>(input: string, params: T): string {
	const urlParts = input.match(/^(https?:\/\/[^/]+)(\/.*)?$/);
	const baseUrl = urlParts?.[1] ?? "";
	const path = urlParts?.[2] ?? input;
	...
	const missingParams = Array.from(
		new Set(path.match(/:([A-Za-z0-9_]+)/g) || []),
	)
		.map((m) => m.slice(1))
	...
	if (missingParams.length > 0) {
		console.error(
			`Missing parameters: ${missingParams.map((p) => `:${p}`).join(", ")}`,
		);
	}
...
const createServicesBuilder = <T extends Record<string, unknown>>(
	...
	function createServices<
		A extends Record<string, ApiConfiguration<T>>,
	>(
		apis: A,
		...
		for (const apiName in apis) {
			...
			services[apiName] = methodsMap as ApiMethods<
				T,
				A[typeof apiName]
			>;
		}
		...
		type PathToServiceMap<S> = {
			[C in ServiceStructure<S> as C["path"]]: {
				fn: C["fn"] extends (...args: infer A) => infer R
					? (...args: A) => R
					: never;
